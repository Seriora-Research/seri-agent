...
- **TUI session** (the primary drive; needs a real PTY — node-pty/ConPTY on Windows):
  ...
  **node, never bun** — bun as the node-pty host on this machine kills every ConPTY child instantly (exit -1073741510 after only `?9001h?1049h`-style enables); under node the same drive works, including from agent tool-shells. Do not set `SERI_DISABLE_MODELS_FETCH=1`; drive the same catalog fetch a user gets. From Git Bash also set `MSYS_NO_PATHCONV=1`, or `type=/exit` reaches the app as `C:/Program Files/Git/exit`. Ready when the transcript shows the splash (the `seri` banner — mark, version, model and directory — and, on an unauthenticated profile, the login picker with footer `↑/↓ move · Enter select · Esc continue`).
- **Non-interactive surfaces** (the exception — plain processes, piped stdio):
  `bun apps/cli/src/cli.ts --selftest`, `--version`, `--help`, and the `serve`/`exec`/`doctor`/`update` verbs, run from the repo root, stdout + `$LASTEXITCODE` captured directly. Nothing else. Any other first word is a task.
...
- **Turn completion:** `"wait=done ·@90000"` — a real turn needs the longer deadline (`@MS` suffix). Once only. A `wait=` scans the whole accumulated byte stream, so a second `wait=done ·` in the same drive matches the *first* turn's line and returns instantly, and the drive races on to quit while turn two is still running. For a multi-turn drive, settle the later turns with `sleep=` or wait on a phrase unique to that turn.
- **Screenshot a drive:** `scripts/frame-from-transcript.mjs <transcript> <out-prefix>` replays the transcript through a minimal VT emulator and writes the final screen as `.txt`, `.json` and `.ansi`; `scripts/frame-to-png.ps1 -Json <out-prefix>.json -Out <out-prefix>.png` paints the grid. Use it for any change whose proof is what the screen looks like. Parsing stops at the alt-screen exit, so the captured frame is the last one the TUI painted — end the drive with a `sleep=` and no `key=ctrl-d` when you want the app at rest rather than its quit screen.
...
**Probes** answer what a driven transcript cannot show, and they assert nothing — a probe run has no pass or fail, only a recording to read. `scripts/probe-mouse.mjs <out-dir> <profile> <step...>` (node, same ConPTY rules as `drive-tui.mjs`) spawns the TUI and writes `raw.bin`, `decoded.txt` and `report.json` into the out-dir: every private-mode enable, disable, save and restore in arrival order (the TUI no longer writes `?1007` itself; a leftover save/restore would still show up here), any OSC 52 payload decoded from base64, and the child's exit code. It takes `drive-tui.mjs`'s whole step vocabulary plus `drag=C1,R1,C2,R2`, `click=C,R` and `wheel=up|down,C,R`, which inject SGR 1006 mouse bytes — the only way to script a mouse against seri. `scripts/probe-clipboard.mjs` (bun, no profile, no PTY) round-trips opentui's native host clipboard and puts back whatever was there; confirm its result with the platform's own reader afterwards, since reading through the same library it wrote with proves nothing on its own. Both were written for `docs/specs/044-tui-selection-copy/research.md` and are the instruments its numbers came from.

**Non-interactive surfaces** are the exception for what has no TUI path: `--version`, `--help`, `--selftest`, and the `serve`/`exec`/`doctor`/`update` verbs (always with `--profile verify-<run-id>` when they touch state). Capture stdout + exit code directly. Check a candidate against `seri --help` before driving it; anything absent there is a task, not a command.
