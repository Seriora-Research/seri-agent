...
- The splash banner spells the product **lowercase** (`seri v0.0.1`, bold). There is no uppercase `SERI` anywhere in the TUI — only `SERI_*` config-key names — and `wait=` matching is case-sensitive, so a `wait=SERI` step matches nothing but a token the drive itself typed.
