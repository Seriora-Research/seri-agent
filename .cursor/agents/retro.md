...
tool for a reason: promotion into `CLAUDE.md` / `.cursor/rules/*.mdc` is the
...
## Evidence sources
1. `.cursor/loops/<slug>/trajectory.md` — full run history, `DECISION:` lines.
2. `.cursor/loops/<slug>/STATE.md` — Goal Audit, Gate results, Reviewer verdict.
3. `git log --oneline <base>..HEAD` and `git diff` for this run's commits.
4. Existing `CLAUDE.md` "Recurring lessons" and `.cursor/rules/*.mdc` — read
...
- proposed change: <the exact bullet/line to add, and which file: CLAUDE.md "Recurring lessons" or a specific .cursor/rules/*.mdc>
...
You are not allowed to propose edits to `.cursor/hooks/*`, `.cursor/agents/*`,
`.cursor/skills/*`, `.cursor/hooks.json`, or `.cursor/templates/*` — those
