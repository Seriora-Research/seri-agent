...
try {
  ...
  if (-not $file) { $file = $json.file_path }
  if (-not $file) { $file = $json.path }
} catch {}

$plan = $false
if ($skill -eq "feature-plan" -or $skill -eq "bugfix-report" -or $skill -eq "research-spec") { $plan = $true }
if ($file -match 'feature-plan\.md$|bugfix-report\.md$|research-spec\.md$') { $plan = $true }
if (-not $plan) { exit 0 }

# --- Resolve THIS session's loop ---
$statePath = $null
if ($sid) {
  foreach ($s in Get-ChildItem -Path ".cursor/loops" -Depth 1 -Filter "SESSION" `
...
if (-not $statePath) {
  $all = @(Get-ChildItem -Path ".cursor/loops" -Depth 1 -Filter "STATE.md" `
    -File -ErrorAction SilentlyContinue)
  if ($all.Count -eq 1) {
    ...
    Write-Error "BLOCKED: $($all.Count) active loops and no SESSION file identifies this one - write the session id to .cursor/loops/<slug>/SESSION at INIT"
    exit 2
  }
}
