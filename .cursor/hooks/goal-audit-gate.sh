...
#
# CHANGED 2026-08-06: this gate used to read `find .cursor/loops -name STATE.md | head -1`,
...
[ -z "$FILE" ] && FILE=$(json_str path)
PLAN=0
case "$SKILL" in
  feature-plan|bugfix-report|research-spec) PLAN=1 ;;
esac
case "$FILE" in
  *feature-plan.md|*bugfix-report.md|*research-spec.md) PLAN=1 ;;
esac
[ "$PLAN" -eq 1 ] || exit 0
...
STATE=""
if [ -n "$SID" ]; then
  for f in .cursor/loops/*/SESSION; do
    [ -f "$f" ] || continue
...
# archived loops under .cursor/loops/_archive/<slug>/.
if [ -z "$STATE" ]; then
  COUNT=$(find .cursor/loops -maxdepth 2 -name "STATE.md" 2>/dev/null | wc -l)
  if [ "$COUNT" -eq 1 ]; then
    STATE=$(find .cursor/loops -maxdepth 2 -name "STATE.md" 2>/dev/null | head -1)
    ...
    echo "BLOCKED: $COUNT active loops and no SESSION file identifies this one — write the session id to .cursor/loops/<slug>/SESSION at INIT" >&2
    exit 2
  fi
fi
