...
#
# Deliberately NOT frozen: CLAUDE.md, .cursor/rules/*.mdc, .cursor/lessons/**,
# .cursor/loops/**, and ordinary project source. Those are the intended
...
FILE=$(echo "$PAYLOAD" | grep -o '"file_path"[[:space:]]*:[[:space:]]*"[^"]*"' | head -1 \
  | sed -E 's/.*:[[:space:]]*"([^"]*)"/\1/')
if [ -z "$FILE" ]; then
  FILE=$(echo "$PAYLOAD" | grep -o '"path"[[:space:]]*:[[:space:]]*"[^"]*"' | head -1 \
    | sed -E 's/.*:[[:space:]]*"([^"]*)"/\1/')
fi
[ -z "$FILE" ] && exit 0
...
#
#   - `-maxdepth 2` keeps archived runs (.cursor/loops/_archive/<slug>/) from counting.
...
ACTIVE=""
for s in $(find .cursor/loops -maxdepth 2 -name "STATE.md" 2>/dev/null); do
