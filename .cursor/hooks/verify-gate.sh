...
# `find .cursor/loops -name … | head -1`, which returns whichever path readdir yields
...
# loops under .cursor/loops/_archive/<slug>/. On ambiguity this gate exits 0 rather than
...
if [ -z "$LOOP_DIR" ]; then
  ...
  if [ "$COUNT" -eq 1 ]; then
    LOOP_DIR=$(dirname "$(find .cursor/loops -maxdepth 2 -name "STATE.md" 2>/dev/null | head -1)")
  else
    exit 0
  fi
fi
