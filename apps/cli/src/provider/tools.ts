...
export function createToolDefinitions(cwd: string) {
  ...
  const globTool = tool({
    ...
    execute: ({ pattern, path }, { abortSignal }) =>
      glob(pattern, { path: resolveAgainstCwd(cwd, path) }, abortSignal),
  });
  ...
  const powershellTool = tool({
    ...
    inputSchema: z.object({ command: z.string(), timeoutMs: z.number().optional() }),
    execute: async ({ command, timeoutMs }, { abortSignal }) =>
      runPowerShell(command, timeoutMs, abortSignal, cwd),
  });
...
// hands the child the parent's own permission mode (subagents/dispatch.ts), so a subagent's `bash`
// re-enters this same gate at the same mode rather than escaping it; `skill` reads one file the
// user themselves put under `.seri/skills/`; `todo` replaces an in-memory checklist derived from
// session messages.
//
...
// rather than in the concurrent-read batch. `todo` is kept out of that batch for the same reason:
// a replace-all snapshot must not race another `todo` call in the concurrent-read set.
//
