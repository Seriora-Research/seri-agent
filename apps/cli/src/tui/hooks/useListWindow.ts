...
export function useListWindow<T>(
  ...
  // still needs its own share of `terminalRows`. See APP_CHROME_ROWS' own comment (format.ts) for
  // why that reservation is unconditional rather than threaded through as props.
